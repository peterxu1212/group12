...
def pre_process_data_files(in_str_file_path, in_int_y_val, in_file_name_list = [], b_guess_when_exception=False, b_test=False):
    ...
    for str_file_name in in_file_name_list:
        ...
        try:
            ...
            #file_handle = open(str_full_file_name, encoding = 'gbk')
...
if b_partial:
    test_files = [f for f in listdir(str_test_files_path)[:i_partial_count] if isfile(join(str_test_files_path, f))]
else:
